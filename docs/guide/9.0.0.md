...
2. `拖拽复制`，**默认关闭**，为减少多次按`Ctrl+C`或者是右键复制所带来的麻烦，CopyTranslator在9.0.0引入一个鼠标拖拽后自动复制选中文字的机制，在打开`拖拽复制`选项后，只需拖拽选中文字，即可复制。
    ...
    <font color="red">启用拖拽复制可能会触发一些意料之外的复制，致使剪贴板数据被覆盖，使用时需注意。当您启用拖拽复制选项后，任何由此导致的损失均由您自行负责</font>
...
4. `设置记忆`，您的设置将自动保存在磁盘中，并在下次启动时自动重新加载。配置文件版本间变动很大，又因为如果配置文件不兼容软件将直接无法使用，影响用户体验，但处理兼容性会占用较长的时间，因此决定**不向前兼容也不向后兼容，每次更新您的旧版本的配置可能会消失。**
