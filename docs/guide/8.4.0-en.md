...
Here is a link to an intro video that my friend recorded(v0.0.6) 2018-09-09 

- [Copytranslator 复译 高容错率实时翻译软件](https://www.bilibili.com/video/av31396414/) by 简约生活干货铺

Latest introduction Video (v0.0.7) 2018.12.06:
- [【超强】CopyTranslator！最适合科研工作者的翻译工具](https://www.bilibili.com/video/av37503818/) by 哼哈未来 

**For a more detailed look at the text tutorial, CopyTranslator has become more and more functional after several iterations(~~Complicated~~)，you can't make full use of it without reading the complete guide, and viewing it will solve 90% of your problems.**

## Basic usage

### Characteristic

1. Check "Listen Clipboard" and CopyTranslator will automatically translate the clipboard contents.

3. `Smart Translator`, `CopyTranslator` will automatically recognize the copied text, and automatically intelligently translate according to the set `source language` and `target language`, that is, if you copy the `source language`, Will be translated as `target language`, copy `target language` will be translated as `source language`. (Don't worry, `Automatic copying` will not interact with this mechanism. But if you also turn on `Detecting language`, then `Intelligent translation will not take effect, only translate your original text into 'target language'. See Q&A for explanation. It is turned on by default and can be turned off in the Settings - Switch interface.

3. `Smart Dictionary`

   A specific translation engine will provide a more detailed explanation of the word and part of the phrase on the focus mode.

    ![iManhV.png](https://s1.ax1x.com/2018/09/26/iManhV.png)


4. `Setting memory`, Your settings will be automatically saved to disk and automatically reloaded the next time you start.  The configuration file version varies greatly, and because the configuration file is incompatible with the software, it will be directly unusable, affecting the user experience, but processing compatibility will take a long time, so it is decided that ** is not forward compatible or backward compatible.  Each time you update your old/new version configuration may disappear.  **
5. In the settings interface, you can **select the interface language** (currently native support: English, Simplified Chinese, Traditional Chinese),it will take effect gradually, no need to restart.
